...
for inx in range(len(l_entangled)):
    """creates entanglements using a combination of the Hadamard gate (circuit.h()), the X-gate, and the CNOT gate (circuit.cx())
    """
    circuit.h(l_entangled[inx][0]) #hardamard gate
    circuit.x(l_entangled[inx][1]) #initializes Player 2's cells to ∣1⟩ using the X-gate 
